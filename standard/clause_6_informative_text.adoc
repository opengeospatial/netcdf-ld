...
This standard does not support nested derivation, an implementation is not required to search the inheritance tree of a defined Class to see if any of the classes it inherits from define themselves as rdfs:subClassOf bald:Resource.
...
* Condition three: the attribute name entity declares and `<rdfs:range>` of `<bald:Resource>` (or uses rdf:subClassOf linkage).
...
then the `coordinates` attribute defined on the votemper variable is interpreted as a varaible to variable reference, creating 4 explicit reference statements with blank nodes representing the bald:Reference instances.

...
While, netCDF allows metadata to be specified in each file, it lacks the mechanisms for specifying metadata that conforms to community or project conventions or standards. 
...
The netCDF-LD standard enables the annotation metadata in netCDF files in a way that is interoperable and reusable. 
netCDF-LD allows existing netCDF files to be interpreted with minimal changes to current netCDF syntax. 
...
Therefore, data providers adopting netCDF-LD are able to create metadata in netCDF files that conform to one or more metadata conventions in a consistent way.
...
Identifiers are the building blocks for defining unambiguous semantics via concept definitions and relationships between concepts.
...
- **Default behaviour while preserving flexibility**. netCDF-LD mechanisms and implementations should have consistent and useful default behaviour/configuration, but allows user-defined configuration if needed.
- **Explicit semantics**. netCDF-LD promotes expressing semantics explicitly even if they implicit in netCDF, e.g. aliases for well-known netCDF attribute names are made with explicit web-resolvable identifiers.
- **Leverage existing Linked Data approaches and tools**. netCDF-LD should adopt leverage existing web and Linked Data approaches tools rather develop bespoke functionality, e.g. query language for netCDF-LD RDF content should use SPARQL.

...
If an array of shape (13,17,7) references an array of shape (17) then the effective reference shape for the relation is a targetRefShape of (1,17,1).
If the same array of shape (13,17,7) references an array of shape (13) then the effective reference shape for the relation is a targetRefShape of (13,1,1).

This explicit reference is crucial for cases where there are unique dimensions of the same size, where inference could be used. Given the generality of these, the standard mandates that targetRefShape shall always be defined, even if it is implicit due to the dimensionality matching.  This is to aid implementations using this information.
...
If an array of shape (13,13,13) references an array of shape (13) then there is no way to infer the correct targetRefShape, it must be specified, e.g. (1,13,1)

If an array of shape (13,17,13,7) references an array of shape (13,3) then both the source and target reference shape must be defined, e.g. sourceRefShape (13,17,13,7,1) targetRefShape (1,1,13,1,3)

The sourceRefShape is optional, it is assumed to be the same as the shape, unless overridden by an explicit sourceRefShape statement.

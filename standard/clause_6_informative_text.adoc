...
Literals are another key part of RDF and are used to capture basic values other than a URI. e.g. strings such as "Gerald", dates such as "1 April 2019", and numbers such as "4897". Literals are associated with a data-type so that it can be parsed. Literals may only appear in the object part of a RDF statement. 
...
Thus, each variable and group within that file has its own identity, that is defined relative to the file identity.
...
Prefix definitions provided explicitly within a file shall not be over-ridden be context files.  Prefixes defined within a file have precedence.
...
Broadcasting enables array which share some dimensions, but have different overall dimensionality, to be interpreted together.  Two arrays may be broadcast if the dimensions they share are ordered the same and extra dimensions can be interpreted unambiguously.
...
In this manner, the source array and the target array are defined in a common dimensionality enabling the shape to be unambiguously defined for broadcasting; i.e.:
...
This reflects the explicit dimension naming within netCDF files. The references are stated explicitly in the file using these named dimensions. This standard does not preserve dimension names within the summary metadata graph but does represent these explicit array to array relationships and their numeracy.
...
Within the Schema.org representation, the DCAT usage to describe the file type and to provide an optional statement to specify the resolvable location of the file object described above is mapped to Schema.org's DataDownload class and its associated properties.
...
For example an implementation may normalise the shared content from a large number of files into a graph which may be defining a data type, which defines all of the statements valid for all instances of that type.  The metadata graph for a single file may then define itself as an instance of that type and hence inherit all of the statements defining that type.  The combination of the type definition and the specific file definition may then be compared to the standardised metadata graph from the single file for informational equivalence.

A common use case is where many files from a simulation run differ only in the time value for the time step and in the data payload encoded within the file.

e.g. A common pattern definition for a large collection of highly similar files may be defined as an identified graph.
...
@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> .
@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .
@prefix this: <http://example.binary-array-ld.net/minVOTemperSharedStatements/> .
@prefix xml: <http://www.w3.org/XML/1998/namespace> .
@prefix xsd: <http://www.w3.org/2001/XMLSchema#> .
...
Then a set of specific instances may be defined with respect to the common metadata pattern.

----
@prefix this: <http://example.binary-array-ld.net/exqf4/minVOTemper200101010000/> .
@prefix bald: <https://www.opengis.net/def/binary-array-ld/> .

this: a <http://example.binary-array-ld.net/minVOTemperSharedStatements/>,
        bald:Container .

this:time a bald:Array ;
    bald:arrayFirstValue "2001-JAN-01 00:00:00" ;
    bald:arrayLastValue "2001-JAN-01 18:00:00" ;
.
...
----
@prefix this: <http://example.binary-array-ld.net/exqf4/minVOTemper200101020000/> .
@prefix bald: <https://www.opengis.net/def/binary-array-ld/> .
...
this:time a bald:Array ;
    bald:arrayFirstValue "2001-JAN-02 00:00:00" ;
    bald:arrayLastValue "2001-JAN-02 18:00:00" ;
.


----

An implementation may then transform these graph pairs into a graph defining the full metadata payload for each file, as per this specification.

Normalisation details are implementation specific, and not standardised by this standard.

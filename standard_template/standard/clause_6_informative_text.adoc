...
In this way, the RDF approach to having objects that are links to subjects, chaining statements into graphs, is implemnted.

References to variables are implemented in nedCDF files by defining the value of an attribute as the name of a variable, or as a space separated set of names of variables, or as a parenthesis bound space separated list of names of variables.
...
An attribute name may be exempted from the process of inferring referernces.  In order for an attribute name to be exempted, the attribute shall provide a downloadable resource from its URI and that resource shall declare an rdfs:range of either rdfs:Literal or skos:Concept.
